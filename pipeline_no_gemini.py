...
# Keywords untuk membantu pemisahan kolom multi-word
...
def extract_kk_pure_pdf(pdf_path):
    ...
    for line in lines:
        ...
        if in_t1: # Prosedur Tabel 1 (Biodata)
        ...
        elif in_t2: # Prosedur Tabel 2 (Status & Ortu)
            cit_idx = -1
            for kw in ["WNI", "WNA"]:
            ...
            if cit_idx != -1:
                ...
                # FIX 1: Pembersihan kebocoran karakter '-' pada Status Kawin
...
def save_to_excel(data_list, output_name, use_mapping=True):
    ...
    for p in data_list:
        ...
        for col in TARGET_COLUMNS:
            ...
            if isinstance(val, str):
                # 1. Hapus tanda hubung HANYA jika itu adalah karakter tunggal '-' 
                # atau berada di ujung teks (trailing dash).
                # Jangan hapus dash di tengah kata seperti LAKI-LAKI.
                temp_val = val.strip()
                if temp_val == "-":
                    clean_val = "-"
                else:
                    # Menghapus dash di akhir dan spasi ganda
                    clean_val = re.sub(r'\s+', ' ', temp_val.rstrip('-').strip()).upper()
                
                # 2. Normalisasi spasi di sekitar slash '/' 
                # (Menangani "SD / SEDERAJAT" vs "SD/SEDERAJAT")
            ...
            if use_mapping and col in MASTER_MAPPINGS:
                ...
                # Normalisasi kunci mapping juga agar sinkron
                # (Opsional: lakukan ini sekali saja di luar loop untuk performa)
    ...
    print(f"✅ Selesai: {output_name} berhasil dibuat.")
# --- CONTOH EKSEKUSI ---

# 1. Jalankan ekstraksi untuk mendapatkan data mentah
all_extracted_data = []
pdf_files = ["KK SARLAN DSN 1.pdf", "KK HASAN DSN 7.pdf", "KK DENI RIYANTO NEW DSN 3.pdf", "KK DENI DSN 3.pdf"]
for f in pdf_files:
    all_extracted_data.extend(extract_kk_pure_pdf(f))

# 2. Toggle OFF: Simpan sebagai teks (misal: "ISLAM", "KAWIN TERCATAT")
save_to_excel(all_extracted_data, "Data_KK_Teks_Mentah.xlsx", use_mapping=False)

# 3. Toggle ON: Simpan sebagai ID angka (misal: 1, 2) untuk kebutuhan database
save_to_excel(all_extracted_data, "Data_KK_Siap_Import_ID.xlsx", use_mapping=True)
